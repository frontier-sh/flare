...
class FlareSettingTab extends PluginSettingTab {
		...
		new Setting(containerEl)
		  ...
		  .addButton(button => button
			...
			.onClick(() => {
			  ...
			  window.open(`${WORKER_URL}/?client=obsidian&state=${state}`, '_blank');
			  this.pollForSetup(state);
			}));
	  } else {
		new Setting(containerEl)
		  .setName('GitHub Status')
		  .setDesc(`Connected to ${this.plugin.settings.repositoryOwner}/${this.plugin.settings.repositoryName}`)
		  .addButton(button => button
	...
	async pollForSetup(state: string) {
	  const checkSetup = async () => {
		try {
		  const response = await fetch(`${WORKER_URL}/api/check-setup?state=${state}`);
		  if (response.ok) {
